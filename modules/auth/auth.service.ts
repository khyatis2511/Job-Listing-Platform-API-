...
const authService = {
  login: async (req: Request) => {
    ...
    try {
      ...
      if (!user || user.userHasPassword.length === 0)
        return returnRes(401, msgs.auth.unauthorized);

      const isMatch = await bcrypt.compare(
        password,
        user.userHasPassword[0].password
      );
      ...
      return returnRes(200, msgs.auth.loggedIn, {
        token,
        userData: {
          ...payload,
